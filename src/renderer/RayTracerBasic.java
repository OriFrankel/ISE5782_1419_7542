...
public class RayTracerBasic extends RayTracerBase {
	...
	private Color calcSpecular(double kS,Vector l,Vector n,Double nl,Vector v,int nShininess,Color intensity) {
		Vector r=l.subtract(n.scale(nl*2));
		double d=-r.dotProduct(v);
		if(d<0)d=0;
		return intensity.scale(java.lang.Math.pow(d,nShininess));
		
	}
	private Color calcDiffusive(double kD,double nl,Color intensity) {
		return intensity.scale((nl>0?nl:-nl)*kD);
	}
}
