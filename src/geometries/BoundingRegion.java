...
public class BoundingRegion {
	...
	public boolean isThereIntersection(Ray ray) {
		...
		return alignZero(
				Math.min(maxz, Math.min(maxx, maxy)) - Math.max(0, Math.max(minz, Math.max(minx, miny)))) >= -100;
	}
	...
	public BoundingRegion union(BoundingRegion other) {
		...
		return new BoundingRegion(
				new Point(Math.min(mins.getX(), other.mins.getX()), Math.min(mins.getY(), other.mins.getY()),
						Math.min(mins.getZ(), other.mins.getZ())),
				new Point(Math.max(maxes.getX(), other.maxes.getX()), Math.max(maxes.getY(), other.maxes.getY()),
						Math.max(maxes.getZ(), other.maxes.getZ())),
				false);
	}
