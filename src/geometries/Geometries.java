...
public class Geometries extends Intersectable {
	...
	public Geometries makeBVH() {
		if(geometries.size()<=3)return this;
		Geometries g1=new Geometries(geometries.get(0)) ,
				g2=new Geometries(geometries.get(geometries.size()-1)),
				g3=new Geometries();
		for(int i=1;geometries.size()-1>i;++i)
			if(geometries.get(i).getBR().getFlag())g3.add(geometries.get(i));
			else if(geometries.get(i).getBR().getMins().distanceSquared(geometries.get(0).getBR().getMins())<
					geometries.get(i).getBR().getMins().distanceSquared(
							geometries.get(geometries.size()-1).getBR().getMins()))
		...
		geometries.add(g1);
		geometries.add(g2);
		if(g3.geometries!=null)geometries.add(g3);
		/*while(geometries.size()>2) {
			int ind1=0,ind2=0;
			double minD2=Double.POSITIVE_INFINITY;
			for(int i=0;geometries.size()>i;++i)
				for(int j=0;i>j;++j){
					double curD2=geometries.get(i).getBR().getMins().
						distanceSquared(geometries.get(j).getBR().getMins());
					if(curD2<minD2) {
						minD2=curD2;
						ind1=i;
						ind2=j;
			...
			Geometries newG=new Geometries(geometries.get(ind1),geometries.get(ind2));
			geometries.remove(ind1);
			geometries.remove(ind2);
			geometries.add(newG);
