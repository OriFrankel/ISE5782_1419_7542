...
class PlaneTests {
	...
	void testGetNormal() {
		...
		assertEquals(v.dotProduct(p2.subtract(p1)), 0, 0.0000001,
				"Plane getNormal wrong result - doesn't orthogonal to vector in the plane");
		assertEquals(v.dotProduct(p3.subtract(p1)), 0, 0.0000001,
				"Plane getNormal wrong result - doesn't orthogonal to vector in the plane");
	...
	void testConstructor() {
		...
		Point point = p.getP0();
		try {
			assertEquals(v.dotProduct(point.subtract(p1)), 0, 0.0000001,
					"wrong plane constructor - first point is not on the plane");
		} catch (IllegalArgumentException e) {

		}
		assertEquals(v.dotProduct(point.subtract(p2)), 0, 0.0000001,
				"wrong plane constructor - second point is not on the plane");
		assertEquals(v.dotProduct(point.subtract(p3)), 0, 0.0000001,
				"wrong plane constructor - third point is not on the plane");
